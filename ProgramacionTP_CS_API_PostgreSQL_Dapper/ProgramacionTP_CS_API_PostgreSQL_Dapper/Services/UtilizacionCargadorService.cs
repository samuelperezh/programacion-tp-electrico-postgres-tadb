    ...
    public class UtilizacionCargadorService
    {
        ...
        public async Task<UtilizacionCargador> CreateAsync(UtilizacionCargador unaUtilizacionCargador)
        {
            ...
            if (estado_hora == "Cargando" || estado_hora_siguiente == "Cargando") {
                throw new AppValidationException($"Ya existe una utilización de cargador en el autobus {unaUtilizacionCargador.Autobus_id}, en el cargador {unaUtilizacionCargador.Cargador_id}, en las horas {unaUtilizacionCargador.Horario_id} y {unaUtilizacionCargador.Horario_id + 1}");
            }
            else if (estado_hora == "Operando" || estado_hora_siguiente == "Operando") {
                throw new AppValidationException($"El autobus con id {unaUtilizacionCargador.Autobus_id}, no se puede poner a cargar en las horas {unaUtilizacionCargador.Horario_id} y {unaUtilizacionCargador.Horario_id + 1} porque está operando");
            }
            ...
            if (utilizacionCargadorExistente.Cargador_id != 0)
                throw new AppValidationException($"Ya existe una utilización de cargador en el autobus {utilizacionCargadorExistente.Autobus_id}, en el cargador {utilizacionCargadorExistente.Cargador_id}, en la horas {utilizacionCargadorExistente.Horario_id} y {utilizacionCargadorExistente.Horario_id + 1}");
